SELECT * FROM (SELECT s.song_name AS nome_musica,
CASE WHEN s.song_name LIKE '%Amar%' THEN REPLACE(s.song_name, 'Amar', 'Code Review')
...
END AS novo_nome
FROM SpotifyClone.songs AS s) AS derived_table
